...
function Pautas() {
    const { error, captureError, clearError } = useError();

    // Función para generar un error simulado
    const triggerError = () => {
        const simulatedError = {
            response: {
                status: 403,
                statusText: 'Forbidden',
                data: { message: 'Ha ocurrido un error inesperado en el servidor' },
            },
        };
        captureError(simulatedError as any);
    };
    ...
    return (
        <div>
                ...
                <div className="">
                    <div className="row card-body">
                        <div className="col-12">
                            ...
                            <button onClick={triggerError} className="btn-primary">
                                Generar Error
                            </button>
                        </div>
                    </div>
            ...
            {error && <ErrorMessage status={error.status} statusText={error.statusText} message={error.message} />}
        </div>
    )
}